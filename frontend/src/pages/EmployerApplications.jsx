...
function EmployerApplications() {
    ...
    const fetchData = async () => {
        try {
            ...
            // Filter applications for this job - convert both to strings for comparison
            const filtered = appRes.data.filter(app => {
                const appJobId = String(app.jobId?._id);
                const currentJobId = String(jobId);
                const matches = appJobId === currentJobId;
                console.log(`App job ID: ${appJobId}, Current job ID: ${currentJobId}, Matches: ${matches}`);
                return matches;
            });

            console.log('Filtered applications:', filtered);

            // Debug: check userId in filtered apps
            filtered.forEach((app, index) => {
              console.log(`App ${index}:`, {
                _id: app._id,
                userId: app.userId,
                userIdValue: app.userId?._id,
                jobId: app.jobId?._id,
              });
            });
            ...
            if (filtered.length === 0) {
                console.warn('⚠️ No applications found for this job');
            }
        } catch (err) {
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="flex gap-2 mb-8 flex-wrap">
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg font-medium ${selectedStatus === ''
                    ...
                    {['applied', 'shortlisted', 'accepted', 'rejected'].map(status => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg font-medium capitalize ${selectedStatus === status
                                    ? 'bg-blue-600 text-white'
                                    : 'bg-white text-gray-700 border border-gray-300'
                                }`}
                        >
                        ...
                        filteredApplications.map(app => (
                            <div key={app._id} className="bg-white rounded-lg shadow-md p-6">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <span className={`px-4 py-1 rounded-full text-sm font-medium capitalize ${app.status === 'applied' ? 'bg-blue-100 text-blue-800' :
                                ...
                                <div className="flex justify-between items-center mb-4">
                                  <StartChatButton 
                                    ...
                                    jobSeekerId={app.userId?._id || app.userId?.id}
                                    jobId={jobId}
                                    applicationId={app._id}
                                    currentUserId={user?._id || user?.id}
                                    otherPersonName={app.userId?.name}
                                  />
                                </div>
                                ...
                                <div className="flex gap-2 flex-wrap">
                                    {['shortlisted', 'rejected', 'accepted'].map(status => (
                                        <button
                                            ...
                                            className={`px-4 py-2 rounded font-medium capitalize ${app.status === status
