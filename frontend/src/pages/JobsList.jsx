...
import { jobsAPI, applicationsAPI, savedJobsAPI } from '../api/auth';
import { MapPin, DollarSign, Briefcase, Search, CheckCircle, Heart } from 'lucide-react';
...
function JobsList() {
  ...
  const fetchSavedJobs = async () => {
    try {
      ...
      const jobIds = savedJobsList.map(item => item.jobId._id);
      setSavedJobIds(jobIds);
    } catch (err) {
      console.error('Failed to fetch saved jobs:', err);
      setSavedJobIds([]); // Reset on error
    }
  };
            ...
            filteredJobs.map(job => {
              ...
              return (
                ...
                >
                  ...
                  {/* Applied Status Button, Save Button & Share */}
                  <div className="mt-4 pt-4 border-t border-gray-200 space-y-3">
                    <div className="flex gap-2">
                      ...
                      >
                        <Heart className={`w-5 h-5 ${savedJobIds.includes(job._id) ? 'fill-current' : ''}`} />
                      </button>
                      <ShareButton job={job} />
                    </div>
                  </div>
